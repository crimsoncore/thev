...
class Program
{
    static void Main()
    {
        ...
        byte[] originalData = new byte[] { 0x48,0x31,0xc9,0x48,0x81,0xe9,
        ...
        byte[] encodedData = new byte[originalData.Length];

        // Encoding process
        for (int i = 0; i < originalData.Length; i++)
        {
            encodedData[i] = (byte)(originalData[i] ^ xorKey[i % xorKey.Length]);
        }

        StringBuilder hex = new StringBuilder();
        for (int i = 0; i < encodedData.Length; i++)
        {
            hex.AppendFormat(i < encodedData.Length - 1 ? "0x{0:x2}, " : "0x{0:x2}", encodedData[i]);
        }

        Console.WriteLine("Encoded Data: " + hex.ToString());
    }
}
